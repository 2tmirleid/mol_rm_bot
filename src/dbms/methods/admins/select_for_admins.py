class SelectForAdmins:
    ...
    async def select_all_events(self, offset=0) -> str:
        return f"""SELECT _id, photo, title, description, date, link, is_active FROM events ORDER BY _id ASC LIMIT 1 OFFSET {offset}"""
    ...
    async def select_all_vacancies(self, offset=0) -> str:
        return f"""SELECT _id, photo, title, description, link, is_active FROM vacancies ORDER BY _id ASC LIMIT 1 OFFSET {offset}"""
    ...
    async def select_all_admins(self, offset=0) -> str:
        return f"""SELECT _id, photo, name, description, phone FROM admins ORDER BY _id ASC LIMIT 1 OFFSET {offset}"""
